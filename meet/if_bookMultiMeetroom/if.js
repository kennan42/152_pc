...
function pushMsg(arg,admin,meetRoomObj,times){
    for(var i in admin){
        var pushArg = {
            appId : global.appId,
            platforms : "0,1",
            title : "您有一条" + util.getMMddHHmmFromTimes(arg.startTime) + "开始" 
                    + meetRoomObj.name + "进行的会议需要审批，要尽快处理喔~",
        ...
        var rtxArg = {"userId":admin[i].userId,"title":pushArg.title,"times":times,"applyUser":applyUserId};
        sendRTXMsg(rtxArg);
    }
}
